...
class LineCommandParser: CommandParser {

    private  val templateRegex =  """[Ll]\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)""".toRegex()

    companion object {
        private val log = LoggerFactory.getLogger(LineCommandParser::class.java)
    }
    ...
    override fun parseCommand(cmd: String): Command {
        ...
        try {
            ...
            return  LineCommand(startWidth = sw.toInt(),
